...
### 6.7 작업 프로세스 가이드 (필수 습관)

#### 6.7.1 작업 시작 전 (Before Starting)

**⚠️ 중요: 코딩 시작 전 반드시 다음 문서들을 읽으세요!**

```bash
# 필수 문서 확인 순서
1. CLAUDE.md         # 개발 규칙 및 제약사항 (이 문서)
2. PROJECTPLAN.md    # 게임 설계 전체 구조
3. TASKS.md          # 현재 작업 목록 및 진행 상황
4. README.md         # 프로젝트 개요 및 실행 방법
```

**읽어야 하는 이유:**
- **CLAUDE.md**: 절대 금지 사항, 핵심 규칙, 코딩 표준
- **PROJECTPLAN.md**: 게임 시스템, 족보 공식, 밸런스, 세계관
- **TASKS.md**: 완료된 작업, 진행 중인 작업, 다음 작업
- **README.md**: 개발 환경 설정, 실행 명령어

**체크리스트:**
```
작업 시작 전:
[ ] CLAUDE.md의 관련 섹션을 읽었는가?
[ ] PROJECTPLAN.md에서 구현할 기능의 사양을 확인했는가?
[ ] TASKS.md에서 이전 작업과의 연관성을 파악했는가?
[ ] 이 작업이 핵심 규칙에 위배되지 않는가?
[ ] 필요한 기술 스택/의존성을 확인했는가?
```

#### 6.7.2 작업 진행 중 (During Work)

**TodoWrite 사용 (필수):**
- 작업이 3개 이상의 단계로 나뉘면 TodoWrite로 추적
- 각 단계 완료 시 즉시 상태 업데이트
- 막히거나 문제 발생 시 Todo에 기록

**코드 작성 시:**
```
작업 중:
[ ] 네이밍 컨벤션을 따르는가? (camelCase, PascalCase 등)
[ ] 에러 처리가 명확한가? (try-catch, 구체적 에러 메시지)
[ ] 주석이 필요한 복잡한 로직이 있는가?
[ ] 성능을 고려했는가? (N+1 쿼리, 무거운 연산 등)
[ ] 보안을 고려했는가? (서버 검증, SQL 인젝션 방지)
```

#### 6.7.3 작업 완료 후 (After Completion)

**⚠️ 중요: 코딩 후 반드시 문서를 최신화하세요!**

**문서 최신화 순서:**

**1. TASKS.md 업데이트 (필수!)**

⚠️ **중요: TASKS.md 편집 시 다음 순서를 정확히 따르세요!**

```bash
# Step 1: 날짜 업데이트
> 최종 업데이트: YYYY-MM-DD  # 오늘 날짜로 변경

# Step 2: 체크박스 업데이트
- [ ] 완료한 작업에 [x] 체크
- [ ] 모든 체크박스가 [x]인지 확인

# Step 3: Phase 완료 시 섹션 이동
1. "진행 중 (In Progress)" 섹션에서 완료된 Phase를 찾음
2. 전체 Phase 블록을 복사 (제목 + 모든 체크박스)
3. "완료된 작업 (Completed)" 섹션의 마지막에 붙여넣기
4. "진행 중" 섹션에서 해당 Phase 삭제

# Step 4: 중복 제거
1. "예정된 작업 (Planned)" 섹션을 확인
2. 완료된 Phase가 아직 남아있으면 제거

# Step 5: 검증
- [ ] 동일한 Phase가 여러 섹션에 중복되지 않는지 확인
- [ ] 모든 Phase 번호가 순서대로 정렬되어 있는지 확인
- [ ] 날짜가 업데이트되었는지 확인
```
...
```markdown
# ❌ 잘못된 예: Phase를 이동하지 않음
## 🔄 진행 중 (In Progress)
### Phase 4: 3D 주사위 시스템 - 완료  ← 완료인데 여기 있으면 안됨!
- [x] Three.js 씬 설정
...
...
# ✅ 올바른 예: Phase를 완료 섹션으로 이동
## ✅ 완료된 작업 (Completed)
### Phase 3: 문서화
...

### Phase 4: 3D 주사위 시스템  ← 완료 섹션으로 이동됨
- [x] Three.js 씬 설정
- [x] 3D 주사위 모델 생성
...

## 🔄 진행 중 (In Progress)
_현재 진행 중인 작업 없음_  ← Phase 4 제거됨
```
...
[ ] 날짜를 오늘로 업데이트했는가?
[ ] 완료한 작업에 [x] 체크했는가?
[ ] Phase 전체 블록을 "완료된 작업" 섹션으로 이동했는가?
[ ] "진행 중" 섹션에서 해당 Phase를 삭제했는가?
[ ] "예정된 작업"에 중복이 없는가?

편집 후:
[ ] 동일 Phase가 여러 섹션에 없는가?
[ ] Phase 번호가 순서대로인가? (1 → 2 → 3 → 4)
[ ] 파일을 저장했는가?
```

**2. 관련 문서 업데이트 (필요 시)
- [ ] PROJECTPLAN.md: 새 기능/시스템 추가 시
- [ ] README.md: 실행 방법/의존성 변경 시
- [ ] docs/*.md: 데이터베이스 스키마 변경 시
...
```
좋은 예:
✅ "Implement 3D dice system with Three.js and Cannon-es"
✅ "Apply balance update: reduce hand powers to ~75%"
✅ "Fix BattleView.vue offline mode using old System A values"
...
[ ] TASKS.md를 업데이트했는가?
[ ] 관련 문서를 최신화했는가?
[ ] Git에 커밋했는가? (명확한 커밋 메시지)
[ ] 원격 저장소에 푸시했는가?
[ ] 다음 작업을 TASKS.md에서 확인했는가?
```
...
#### 6.7.5 자주 하는 실수 (Common Mistakes)

**❌ 문서를 읽지 않고 코딩 시작**
```
문제: 규칙 위반, 중복 작업, 잘못된 구현
해결: 작업 전 항상 CLAUDE.md, PROJECTPLAN.md, TASKS.md 읽기
```

**❌ TASKS.md를 업데이트하지 않음**
```
문제: 진행 상황 파악 불가, 중복 작업 발생
해결: 작업 완료 즉시 체크박스 업데이트
```

**❌ 커밋 메시지가 불명확**
```
나쁜 예: "update", "fix", "wip"
좋은 예: "Fix BattleView.vue offline mode using old System A values"
```

**❌ 문서는 업데이트했지만 커밋을 안 함**
```
문제: 로컬에만 변경사항 존재, 다른 환경에서 확인 불가
해결: 문서 업데이트 후 즉시 커밋 및 푸시
```

**❌ 여러 기능을 한 번에 커밋**
```
문제: 커밋 히스토리 복잡, 롤백 어려움
해결: 기능별로 커밋 분리 (예: 3D 주사위, 밸런스 조정, 버그 수정)
```

#### 6.7.6 작업 프로세스 요약

```
📖 읽기 → 💻 코딩 → 📝 문서화 → 🔄 커밋 → ⬆️ 푸시 → 🔁 반복
```

**"문서를 읽고, 코드를 쓰고, 문서를 업데이트하고, 커밋하고, 푸시한다."**

---

## 📝 문서 정보

- **문서 버전**: 2.1.0 (작업 프로세스 추가)
- **최종 수정일**: 2026-01-31
- **대상**: 프로젝트 개발 에이전트
- **목적**: 개발 시 준수해야 할 핵심 규칙 및 작업 프로세스 정의
